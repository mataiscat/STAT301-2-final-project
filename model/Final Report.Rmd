...
Models 2 and 3 produced the smallest test error of 0.233 using KNN with folds of 2.
...
Here, models 1 and 2 produce the smallest test error of 0.20 using generalized linear model; however, I will favor model 2 with the theoretical variables such that it uses only a subset of all the variables form model 1.
...
Now, we begin to see the difference of these imputed variables, the result from model 2 using LDA produce the new lowest test error of 0.166, and from the confusion matrix, we see much fewer over-estimation for patient without heart disease to be predicted as presence (4/20).
...
However, QDA shows that the result from model 3 produce the same test error of 0.166, and same result of the confusion matrix as model 2 using LDA.
...
```{r}
heart_rf_class <- heart_db %>% 
  crossing(mtry = 1:(ncol(train) - 1)) %>% # list all mtry values
  # Fit the model for each mtry value
  mutate(model = map2(.x = train, .y = mtry, 
                      .f = function(x, y) ranger(target ~ ., 
                                                 mtry = y, 
                                                 data = x, 
         ...
         train_misclass = map2(model, train, misclass_ranger, outcome = "target"),
         test_misclass = map2(model, test, misclass_ranger, outcome = "target"), 
...
From model building, we see LDA and QDA models provide the most effective fit for our data producing lowest test error than other methods explored. From imputing the `age` and `cp` variables as categorical, we improve the model from lowest test error of 0.23 to 0.16. A next step will be continue exploring the possible imputation for other variables such as `slope`, `ca`, and `thal` and their effect on the models. In the future as we learn more classification models, we can apply more methods such as random forest to see if we can improve the result a lot more. The variables selected from theortically related to heart disease I think fits the best for most models explored here; and I think it makes sense as most doctors uses these measures for example exercise to evaluate the patients.
